...
/**
 * SDK instantiation in client auth enabled mode. This mechanism is preferred for integration of the SDK with web app in
 * production stage.
 * When client authentication has been enabled, client authentication is enforced by signed JWT tokens in addition to
 * the unblocked lists.
 * When the SDK needs to establish a connection with the ODA server, it first requests a JWT token from the client and
 * then sends it along with the connection request. The ODA server validates the token signature and obtains the claim
 * set from the JWT payload to verify the token to establish the connection.
...
function initSdk(name) {
     ...
     * In client auth enabled mode, 'URI' and 'clientAuthEnabled: true' must be passed.
     * The channelId and userId is picked up from the signed JWT claim, any value for them passed
     * directly in the configuration settings is ignored.
     */
    var chatWidgetSettings = {
        URI: '<URI>',  // ODA URI, only the hostname part should be passed, without the https://
        clientAuthEnabled: true,     // Enables client auth enabled mode of connection if set true
